...
- AWS 프리 티어 활용하여 실제 서비스 배포
- 배포 방식: 파일 업로드, Docker 컨테이너 등 다양한 방식 지원

### 10. Spring Security 고도화
...
| 분야           | 기술                                          |
| :------------: | ------------------------------------------- |
| **Backend**  | Spring Boot, JPA (ORM 프레임워크), OpenFeign, QueryDSL (동적 쿼리 생성), Specification (JPA에서 제공하는 동적쿼리 생성 인터페이스) Kibana(Elasticsearch 데이터 시각화 도구) |
| **Security** | Spring Security, JWT (토큰 기반 인증) |
| **Database** | PostgreSQL, Elasticsearch (분산검색 및 분석엔진), OpenSearch(실시간 검색 및 로그 분석도구) |
| **Cloud**    | AWS EC2 (가상 서버 호스팅) |
| **Chache** | Redis (캐싱 및 세션 관리), Spring chache(Spring의 캐싱 추상화), Jedis(Redis의 클라이언트 라이브러리)
...
| **역할**                |   **담당자**        |  **세부 업무**                     |
|-------------------------|:---------------------:|------------------------------------|
| **리뷰 및 평점 관리**              | 한석규      | - 리뷰 시스템 CRUD 설계 및 구현 <br> - 1~5점까지의 평점을 기록하는 기능 구현  <br> - 주문 한건당 리뷰 한건 작성가능 <br> - softDelete 구현 <br> - 리뷰 필터링, 정렬 기능 구현 <br> - 주문 검색 시 음식점 리뷰 리스트 표시|
| **주문(온라인,오프라인) 관리** | 강혜주      | - 주문 시스템 CRUD 설계 및 구현 <br>- 온라인/오프라인 주문 흐름 구현 <br> - 주문 생성 후 5분 이내 취소 가능기능 구현 <br> - softDelete 구현 <br> - 주문 상태 추적 관리 <br> - 주문 내역 조회 필터링, 정렬 기능 구현 <br> - 결제내역 관리 |
| **회원주소 및 지역코드 관리**   | 문준영      | - 회원 배송지 주소 관리 시스템 CRUD 설계 및 구현 <br> - 배송지역 확장성을 고려한 Elasticsearch 사용|
| **가게,음식 카테고리 관리**     | 신다은      | - 가게, 음식 카테고리 시스템 CRUD 설계 및 구현 <br> - 가게,음식 카테고리 관리 시 관리자 권한만 접근가능하도록 구현 <br> - softDelete 구현 <br> - Specification을 사용한 검색 필터링 기능 구현
...
| **클라우드 배포**    | 한석규      | - EC2 인스턴스 생성 및 초기설정 <br> - 보안 그룹 설정 (포트 개방 등) <br> - 애플리케이션 및 필요한 라이브러리 설치 <br> - RDS 인스턴스 생성 및 설정 <br> - RDS 보안 그룹 설정 (EC2와의 연결 허용) <br> - 애플리케이션의 환경 변수 설정 (RDS 데이터베이스 연결 정보 포함) <br> - 애플리케이션 배포 (배포 자동화 스크립트 적용 등) |
